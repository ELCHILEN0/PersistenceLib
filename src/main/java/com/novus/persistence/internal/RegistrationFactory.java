...
public class RegistrationFactory {
	...
	private static HashMap<Field, ColumnRegistration>		columnRegistrations	= new HashMap<Field, ColumnRegistration>();
	private static HashMap<Class<?>, ColumnRegistration>	idRegistrations		= new HashMap<Class<?>, ColumnRegistration>();

	/**
	 * Returns a TableRegistration object based on a properly annotated Java
	 * class.
	 * 
	 * @param clazz
	 ...
	 */
	public static TableRegistration getTableRegistration(Class<?> clazz) throws TableRegistrationException, ColumnRegistrationException {
		if (tableRegistrations.containsKey(clazz)) {
			return tableRegistrations.get(clazz);
		}

		if (!(clazz.isAnnotationPresent(Table.class))) {
			throw new TableRegistrationException("Class '" + clazz.getCanonicalName() + "' does not have a valid Table annotation.");
		}

		if (RegistrationFactory.getIdRegistration(clazz) == null) {
			throw new TableRegistrationException("Class '" + clazz.getCanonicalName() + "' does not have a valid Id annotation.");
		}
	...
	public static ColumnRegistration getColumnRegistration(Field field) throws ColumnRegistrationException {
		...
		if (!(field.isAnnotationPresent(Column.class))) {
			throw new ColumnRegistrationException("Field '" + field.getClass().getCanonicalName() + "' does not have a Column annotation present.");
		}

		Pattern pattern = Pattern.compile("^[a-zA-Z0-9_]*$");
		if (!(pattern.matcher(field.getAnnotation(Column.class).name()).find())) {
			throw new ColumnRegistrationException("Field '" + field.getClass().getCanonicalName() + "' column name has illegal characters in it. The name is limited to alphanumeric characters and '_'.");
		}

		ColumnRegistration registration = new ColumnRegistration(field, field.getAnnotation(Column.class));
	 ...
	 * @throws ColumnRegistrationException
	 */
	public static List<ColumnRegistration> getColumnRegistrations(Class<?> clazz) throws ColumnRegistrationException {
	...
	/**
	 * Returns the ColumnRegistration that is designated as the Id of a Java
	 * class.
	 * 
	 * @param clazz
	 ...
	 * @throws ColumnRegistrationException
	 */
	public static ColumnRegistration getIdRegistration(Class<?> clazz) throws ColumnRegistrationException {
